...
| engine | A technology-specific analyzer layer activated only when repository evidence exists (see `docs/TECHNOLOGY_ENGINES.md`). |
...
| policy rule | A BugHunt-native AST rule shipped in `src/bughunt/policy_scan.py` (see `docs/DEFAULT_RULES.md`). |
| seam | A contract boundary between components checked for drift (see `docs/SEAM_CORRECTNESS.md`). |
