...
EXCLUDED = {
    ...
    ".tox", ".nox", "build", "dist", ".bughunt", "mutants", "__pycache__",
}

SECRET_NAME = re.compile(r"(?:secret|token|password|passwd|api[_-]?key|private[_-]?key|access[_-]?key|client[_-]?secret)", re.I)
...
CONFIG_MODULE = re.compile(r"(?:^|[._/])(config|settings|constants|defaults|cli|options)(?:[._/]|$)", re.I)
UPPER_LAYER = {"api", "apis", "route", "routes", "web", "ui", "service", "services", "domain", "core", "application", "usecase", "usecases", "handler", "handlers", "controller", "controllers"}
PERSIST_IMPL = {"sqlite", "postgres", "postgresql", "mysql", "sqlalchemy", "peewee", "orm", "mongo", "mongodb", "redis"}
ROUNDTRIP_PAIRS = (("export", "import"), ("backup", "restore"))
PLACEHOLDER_VALUES = {
    "", "changeme", "change-me", "example", "example-value", "placeholder",
    "your-value-here", "your_value_here", "todo", "unset", "none", "null",
    "<required>", "required", "replace-me", "replace_me",
}
...
def _unit_for(name: str) -> str | None:
    n = name.lower()
    suffixes = {
        "_ms": "milliseconds", "_millis": "milliseconds", "_milliseconds": "milliseconds",
        "_sec": "seconds", "_secs": "seconds", "_seconds": "seconds",
        "_min": "minutes", "_mins": "minutes", "_minutes": "minutes",
        "_hours": "hours", "_bytes": "bytes", "_kb": "kilobytes", "_mb": "megabytes",
        "_percent": "percent", "_percentage": "percent", "_port": "TCP/UDP port",
    }
...
def _range_for_variable(tree: ast.AST, variable: str) -> str | None:
    ...
    for node in ast.walk(tree):
        ...
        for left, op, right in zip(operands, node.ops, operands[1:]):
            ...
            if isinstance(left, ast.Name) and left.id == variable and right_num is not None:
                symbol = {
                    ast.Gt: ">", ast.GtE: ">=", ast.Lt: "<", ast.LtE: "<=", ast.Eq: "==",
                }.get(type(op))
                if symbol:
                    constraints.append(f"{symbol}{right_num}")
            elif left_num is not None and isinstance(right, ast.Name) and right.id == variable:
                symbol = {
...
def discover_env_uses(root: Path, source_paths: Iterable[str]) -> list[EnvUse]:
    ...
    for path in _files(root, source_paths):
        ...
        for node in ast.walk(tree):
            ...
            inferred = "str"
            if isinstance(node, ast.Subscript) and _call_name(node.value) in {"os.environ", "environ"}:
            ...
            if isinstance(parent, ast.Call):
                wrapper = _call_name(parent.func).split(".")[-1]
            ...
            out.append(
                EnvUse(
                    ...
                    unit=_unit_for(key),
                    accepted_range=_range_for_variable(tree, _assigned_name(node, parents) or ""),
...
def _looks_real_secret(name: str, value: str) -> bool:
    ...
    if low in PLACEHOLDER_VALUES or low.startswith("example_") or low.startswith("test_"):
        return False
    ...
    if re.match(r"^(?:sk-|ghp_|github_pat_|xox[baprs]-|AKIA)[A-Za-z0-9_\-]{12,}$", value):
        return True
    # Secret-named variables should not ship plausible long credentials.
    if SECRET_NAME.search(name) and len(value) >= 16 and re.fullmatch(r"[A-Za-z0-9_./+=:@\-]+", value):
        return True
    return False


def ensure_env_example(root: Path, source_paths: Iterable[str]) -> tuple[Path | None, list[EnvUse]]:
    ...
    lines = [begin, "# Generated from static environment-variable use. Never contains real secrets."]
    for item in uses:
        if item.name in existing_values and item.name not in _parse_managed_keys(existing_text, begin, end):
            continue
    ...
    inventory.write_text(json.dumps({"schema_version": 1, "variables": [asdict(x) for x in uses]}, indent=2) + "\n")
    return path, uses
...
def _literal_operational_kwargs(tree: ast.AST) -> Iterable[tuple[ast.keyword, str, object]]:
    for node in ast.walk(tree):
        ...
        for keyword in node.keywords:
            ...
            if isinstance(value, (str, int, float, bool)) and not isinstance(value, str) or isinstance(value, str) and value:
...
def _signature_annotations(node: ast.FunctionDef | ast.AsyncFunctionDef) -> list[tuple[ast.AST, str]]:
...
def _scan_source_policies(root: Path, source_paths: Iterable[str]) -> list[PolicyFinding]:
    ...
    for path in _files(root, source_paths):
        ...
        for jump in _finally_jump_nodes(tree):
            findings.append(PolicyFinding(
                rel, getattr(jump, "lineno", 1), getattr(jump, "col_offset", 0) + 1, "BHCTRL001",
        ...
        for import_node in tree.body:
            if isinstance(import_node, ast.Import):
                for alias in import_node.names:
                    if {x.lower() for x in re.split(r"[._\-]", alias.name)} & PERSIST_IMPL:
                        persistence_aliases.add(alias.asname or alias.name.split(".")[0])
            elif isinstance(import_node, ast.ImportFrom) and import_node.module:
                if {x.lower() for x in re.split(r"[._\-]", import_node.module)} & PERSIST_IMPL:
                    persistence_aliases.update(alias.asname or alias.name for alias in import_node.names)
        ...
        for node in tree.body:
            ...
            for name in imported:
                ...
                if importer_tokens & UPPER_LAYER and imported_tokens & PERSIST_IMPL:
                    findings.append(PolicyFinding(
                        rel, getattr(node, "lineno", 1), 1, "BHPERS001",
                ...
                if any(part.startswith("_") and not part.startswith("__") for part in name.split(".")):
                    findings.append(PolicyFinding(
                        rel, getattr(node, "lineno", 1), 1, "BHARCH001",
            ...
            if isinstance(node, ast.ImportFrom):
                for alias in node.names:
                    if alias.name.startswith("_") and not alias.name.startswith("__"):
                        findings.append(PolicyFinding(
                            rel, getattr(node, "lineno", 1), 1, "BHARCH002",
        ...
        if importer_tokens & UPPER_LAYER and persistence_aliases:
            for fn in tree.body:
                ...
                for ann_node, annotation in _signature_annotations(fn):
                    ...
                    if root_name in persistence_aliases or any(re.search(rf"\b{re.escape(alias)}\b", annotation) for alias in persistence_aliases):
                        findings.append(PolicyFinding(
                            rel, getattr(ann_node, "lineno", fn.lineno), 1, "BHPERS002",
            ...
            for node in tree.body:
                ...
                for name in targets:
                    if CONFIG_NAME.search(name) and not name.isupper():
                        findings.append(PolicyFinding(
                            rel, getattr(node, "lineno", 1), 1, "BHCFG004",
        ...
        if not CONFIG_MODULE.search(rel):
            for keyword, name, value in _literal_operational_kwargs(tree):
                ...
                findings.append(PolicyFinding(
                    rel, getattr(keyword, "lineno", 1), getattr(keyword, "col_offset", 0) + 1, "BHCFG005",
                    f"operational keyword `{name}={value!r}` is hard-coded at a call site; prefer typed/config-file/env/CLI/generated configuration unless this value is a genuine invariant",
                    "note",
                ))
    return findings
...
def _scan_test_policies(root: Path, test_paths: Iterable[str]) -> list[PolicyFinding]:
    ...
    for path in _files(root, test_paths):
        ...
        for fn in ast.walk(tree):
            ...
            mock_detail_refs = [c for c in calls if c.endswith(("assert_called_once_with", "assert_called_with"))]
            segment = ast.get_source_segment(source, fn) or ""
            if exact_sequence or ".mock_calls" in segment or ".call_args_list" in segment:
                findings.append(PolicyFinding(
                    rel, fn.lineno, 1, "BHTEST002",
            ...
            patch_count = sum(
                1 for c in calls
                if c.endswith(("patch", "patch.object", "mocker.patch", "monkeypatch.setattr")) or "patch" in c.split(".")[-1].lower()
            )
            if patch_count >= 5 and len(mock_detail_refs) >= 2:
                findings.append(PolicyFinding(
                    rel, fn.lineno, 1, "BHTEST004",
            ...
            for inner in ast.walk(fn):
                if not isinstance(inner, ast.Assert) or not isinstance(inner.test, ast.Compare):
                    continue
                operands = [inner.test.left, *inner.test.comparators]
                long_literal = next((x.value for x in operands if isinstance(x, ast.Constant) and isinstance(x.value, str) and len(x.value) >= 1000), None)
                if long_literal is not None:
                    findings.append(PolicyFinding(
                        rel, inner.lineno, 1, "BHTEST003",
                ...
                string_literals = [x.value for x in operands if isinstance(x, ast.Constant) and isinstance(x.value, str)]
                other_texts = []
                ...
                if any(len(value) >= 200 for value in string_literals) and any(
                    re.search(r"(?:generated|source|code|script|sql|html|render)", text) for text in other_texts
                ):
                    findings.append(PolicyFinding(
                        rel, inner.lineno, 1, "BHTEST005",
...
def _looks_like_user_format_operation(node: ast.FunctionDef | ast.AsyncFunctionDef) -> bool:
    param_names = {
        arg.arg.lower()
        for arg in (node.args.posonlyargs + node.args.args + node.args.kwonlyargs)
    }
    if param_names & {"path", "file", "filename", "stream", "archive", "backup", "data", "content", "payload"}:
        return True
    io_markers = (
        "open", "read_text", "write_text", "read_bytes", "write_bytes",
        "json.dump", "json.dumps", "json.load", "json.loads",
        "yaml.", "toml", "pickle.", "csv.", "zipfile.", "tarfile.", "encode", "decode",
...
def _roundtrip_inventory(root: Path, source_paths: Iterable[str]) -> tuple[list[tuple[str, str, str, int]], list[tuple[str, str, str, int]]]:
            ...
            for left_token, right_token in ROUNDTRIP_PAIRS:
                for name, node in functions.items():
                    ...
                    if left_token in parts:
                        ...
                        counterpart = "_".join(parts[:idx] + [right_token] + parts[idx + 1 :])
                    elif right_token in parts:
                        idx = parts.index(right_token)
                        matched_token = right_token
                        counterpart = "_".join(parts[:idx] + [left_token] + parts[idx + 1 :])
                    if counterpart is None:
                        continue
                    qualified = f"{owner}.{name}" if owner else name
                    qualified_counterpart = f"{owner}.{counterpart}" if owner else counterpart
                    if counterpart in names:
                        # Record each pair exactly once from the outward/export side.
                        if matched_token == left_token:
                            pairs.append((qualified, qualified_counterpart, rel, node.lineno))
                    elif _looks_like_user_format_operation(node):
                        subject_tokens = {part for part in parts if part not in {left_token, right_token, ""}}
                        infrastructure_words = {"config", "configured", "configuration", "linter", "module", "package", "plugin", "library", "lib", "path"}
                        if not (subject_tokens & infrastructure_words):
                            orphans.append((qualified, qualified_counterpart, rel, node.lineno))
    return pairs, orphans


def _roundtrip_pairs(root: Path, source_paths: Iterable[str]) -> list[tuple[str, str, str, int]]:
    return _roundtrip_inventory(root, source_paths)[0]

def _scan_roundtrip_coverage(root: Path, source_paths: Iterable[str], test_paths: Iterable[str]) -> list[PolicyFinding]:
    tests: list[set[str]] = []
    for path in _files(root, test_paths):
        try:
            tree = ast.parse(path.read_text(errors="replace"))
    ...
    for operation, expected, rel, line in orphans:
        ...
        token = next((tok for pair in ROUNDTRIP_PAIRS for tok in pair if tok in leaf.split("_")), "export/import")
        out.append(PolicyFinding(
            rel, line, 1, "BHRT002",
...
def scan(root: Path, source_paths: Iterable[str], test_paths: Iterable[str]) -> list[PolicyFinding]:
    findings: list[PolicyFinding] = []
    uses = discover_env_uses(root, source_paths)
    ...
    if uses and not env_file.exists():
        first = uses[0]
        findings.append(PolicyFinding(first.path, first.line, 1, "BHCFG001", "environment-driven configuration detected but `.env.example` is missing", "warning"))
    elif uses:
        for item in uses:
            if item.name not in env_values:
                findings.append(PolicyFinding(item.path, item.line, 1, "BHCFG002", f"environment variable `{item.name}` is used in code but missing from `.env.example`", "warning"))
    if env_file.exists():
        for key, value in env_values.items():
            if _looks_real_secret(key, value):
                findings.append(PolicyFinding(".env.example", 1, 1, "BHCFG003", f"`.env.example` contains a value for secret-like variable `{key}` that looks like a real credential; examples must never contain real secrets", "error"))
