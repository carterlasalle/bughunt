...
def _mypy_config(root: Path, python_paths: list[str], python_version: str) -> str:
    ...
    enabled = "deprecated, redundant-expr, possibly-undefined, truthy-bool, truthy-iterable, ignore-without-code, unused-awaitable, explicit-override, mutable-override, explicit-any, exhaustive-match"
...
"""
    ...
    return ConfigArtifact(
        ...
        "bughunt.toml",
        "READY",
        "cyclomatic, cognitive, function/file LOC, ABC, and built-asset size budgets generated",
    )
...
def _ensure_mutmut_config(
    ...
    if isinstance(existing, dict):
        ...
        if gaps:
            return ConfigArtifact(
                ...
                "pyproject.toml",
                "REVIEW",
                "existing user-owned [tool.mutmut] preserved but coverage is incomplete: "
                + "; ".join(gaps),
            )
        return ConfigArtifact(
            "mutmut",
    ...
    return ConfigArtifact(
        ...
        "pyproject.toml",
        "READY",
        "added managed [tool.mutmut] source/test scoping; original backed up under .bughunt/backups",
    )
...
def configure_custom_checks(root: Path, targets: Iterable[object]) -> ConfigArtifact:
    ...
    lines = [
        begin,
        "# Regenerated by `bughunt configure --auto`; user-authored checks outside this block are preserved.",
    ]
    for check in checks:
        lines.extend(
            [
...
def _configure_technology_overlays(
    ...
    if inv.has("cpp"):
        put(
            ...
            "clang-tidy.yaml",
            _clang_tidy_config(),
            "clang-analyzer + bugprone + concurrency checks; requires compilation database to execute",
        )
    if inv.has("javascript-typescript"):
        put(
            ...
            "correctness+suspicious strict; pedantic/restriction/perf warnings; style disabled",
        )
        put(
            "ESLint",
            "eslint.config.mjs",
            _eslint_config(inv.has("typescript")),
            "correctness-first JS plus strict type-aware typescript-eslint when TypeScript is detected; project-owned ESLint configs remain supported",
        )
    if inv.has("protobuf"):
        put(
            "Buf",
            "buf.yaml",
            _buf_config(),
            "STANDARD lint + FILE breaking policy; used as an overlay when the repository has no stronger own policy",
        )
    if inv.has("php"):
        put(
            "PHPStan",
...
def configure_all(
    ...
    *,
    ...
    put(
        ...
        "strict plus Any ban, unreachable analysis, and optional correctness error codes",
    )
    put(
        "ty",
        "ty.toml",
        _ty_config(),
        "all rules=error; strict equality/generic narrowing; generic type:ignore disabled",
    )
    put(
        "Pyrefly",
        "pyrefly.toml",
        _pyrefly_config(config_dir, root, python_paths, source_paths, python_version),
        "preset=all; no import-to-Any escape hatches; unannotated bodies checked; constants treated final",
    )
...
""",
    ...
    put(
        ...
        _hypothesis_plugin(),
        "2,000 examples, 100 state steps, shrinking + multiple bug reporting, no health-check suppression",
    )
    put(
        "coverage.py",
        "coverage.ini",
    ...
    put(
        ...
        "all generation modes, shrinking, x00/extra params, continue-on-failure, auto workers",
    )
    ...
    artifacts.extend(
        [
            ConfigArtifact(
                ...
                "bughunt.toml",
                "READY",
                "asserts + PEP316 + Deal + icontract; extended solver budgets in deep/all",
            ),
            ConfigArtifact(
                "Deal",
                "bughunt.toml",
            ...
            ConfigArtifact(
                ...
                "built-in",
                "READY",
                "env contract/secrets, persistence-boundary coupling, export round-trip coverage, and implementation-coupled test heuristics",
            ),
            ConfigArtifact(
                "BugHunt complexity",
                "bughunt.toml",
                "READY",
                "cyclomatic + LOC + ABC + built asset budgets; complements Ruff/Pylint/Complexipy/Lizard/Radon",
            ),
            ConfigArtifact(
                "deptry",
                "bughunt.toml",
                "READY",
                "all dependency rules; first-party source roots with runtime/generated exclusions",
            ),
            ConfigArtifact(
                "Atheris",
