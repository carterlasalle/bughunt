...
def _ruff_config(
    ...
    # convention, and expected values are fixture data, not magic. Everything
    # else (annotations, line length, imports, security, bug rules) still applies.
    test_ignores = ", ".join(
        json.dumps(x)
        for x in [
            "S101",
...
def _pylint_config(
    *, extra_plugins: Iterable[str] = (), for_tests: bool = False
) -> str:
    ...
    # like '.py'/'__init__' and small bounds are not magic; operational
    # numbers belong to policy budgets), invalid-name (ast.NodeVisitor
    # mandates visit_CamelCase), import-outside-toplevel (deferred imports
    # are the startup/cycle/optional-dep pattern; importtime owns startup),
    # consider-using-assignment-expr (walrus-or-not is style, not bugs),
    # broad-exception-caught (ruff BLE001 owns the verdict; every site keeps
    # its per-site isolation noqa), duplicate-code (cross-module micro-literals
    # like EXCLUDE sets and tool lists; extraction would couple modules for
    # trivia).
    strict_disable = (
        "use-implicit-booleaness-not-comparison-to-zero,"
        "missing-module-docstring,"
...
def _bandit_config() -> str:
    ...
    return """# Auto-generated by BugHunt. Every installed Bandit check is enabled.
exclude_dirs:
  - .git
  - .venv
  - venv
