...
def _basedpyright_config(config_dir: Path, root: Path, python_paths: list[str], python_version: str) -> str:
...
def _mypy_config(root: Path, python_paths: list[str], python_version: str) -> str:
    ...
    exclude = r"(?x)(^|/)(\.venv|venv|node_modules|build|dist|\.bughunt/runtime|\.bughunt/cache|\.bughunt/reports|mutants)(/|$)"
    return f'''# Auto-generated by BugHunt. PARANOID/MAX-RECALL profile.
[mypy]
...
def _pyrefly_config(
    ...
    if interpreter.exists():
        lines.append(f"python-interpreter-path = {json.dumps(_rel(config_dir, interpreter))}")
    return "\n".join(lines) + "\n"
...
def _pylint_extensions() -> list[str]:
    ...
    try:
        import pylint.extensions as extensions  # type: ignore[import-not-found]
    except Exception:
        return []
    result: list[str] = []
...
language: Python
severity: error
message: Exception is silently swallowed; this hides failures and can corrupt control flow.
rule:
  any:
    - pattern: |
        try:
...
language: Python
severity: error
message: return/break/continue inside finally can suppress exceptions or override control flow.
rule:
  any:
    - pattern: |
        try:
...
def _semgrep_correctness_rules() -> str:
  ...
  - id: bughunt.cached-generator
    message: Caching a generator reuses the same consumable iterator; later cache hits can silently return an exhausted generator.
    languages: [python]
    severity: ERROR
  ...
  - id: bughunt.unconsumed-threadpool-map
    message: ThreadPoolExecutor.map results are never consumed, so worker exceptions may never be observed by the caller.
    languages: [python]
    severity: ERROR
    metadata: {category: correctness, confidence: high}
    patterns:
  ...
  - id: bughunt.dict-delete-during-iteration
    message: Deleting entries from the dictionary being iterated can raise RuntimeError or invalidate iteration semantics.
    languages: [python]
    severity: ERROR
    metadata: {category: correctness, confidence: high}
    pattern-either:
  ...
  - id: bughunt.list-mutation-during-iteration
    message: Mutating the list currently being iterated changes iterator behavior and commonly skips, repeats, or indefinitely extends work.
    languages: [python]
    severity: WARNING
  ...
  - id: bughunt.sync-sleep-in-async
    message: time.sleep inside async code blocks the event loop; use an async wait or move blocking work off the loop.
    languages: [python]
    severity: ERROR
    metadata: {category: correctness, confidence: high}
    patterns:
  ...
  - id: bughunt.file-rebound-before-close
    message: This file handle is overwritten before it is closed, leaking the original resource until garbage collection.
    languages: [python]
    severity: ERROR
    metadata: {category: correctness, confidence: high}
    patterns:
  ...
  - id: bughunt.named-tempfile-name-before-flush
    message: A NamedTemporaryFile path is consumed after writes but before flush/close, so another reader may observe incomplete data.
    languages: [python]
    severity: ERROR
    metadata: {category: correctness, confidence: high}
    patterns:
...
def _import_linter_config(package_roots: list[str]) -> str | None:
    ...
    root_value = json.dumps(package_roots[0]) if len(package_roots) == 1 else "[" + ", ".join(json.dumps(x) for x in package_roots) + "]"
    lines = [
        "# Auto-generated by BugHunt. Generic invariant only: no dependency cycles among package siblings.",
        "[tool.importlinter]",
        f"{key} = {root_value}",
...
def _pysa_files(root: Path, source_paths: list[str], python_version: str) -> tuple[str, str, str]:
...
def _complexipy_config(source_paths: list[str]) -> str:
    ...
    return f'''# Auto-generated by BugHunt. Cognitive complexity is independent of McCabe CC.
paths = [{paths}]
...
def _ensure_complexity_budgets(root: Path) -> ConfigArtifact:
    ...
    if "complexity" in parsed:
        return ConfigArtifact("complexity budgets", "bughunt.toml", "EXISTING", "existing [complexity] budgets preserved")
    block = '''
...
'''
    ...
    return ConfigArtifact("complexity budgets", "bughunt.toml", "READY", "cyclomatic, cognitive, function/file LOC, ABC, and built-asset size budgets generated")
...
def _normalized_paths(value: object) -> list[str]:
    if isinstance(value, str):
        ...
        return []
    return [item.replace("\\", "/").strip().removeprefix("./").rstrip("/") or "." for item in values]
...
def _ensure_mutmut_config(root: Path, source_paths: list[str], test_paths: list[str]) -> ConfigArtifact:
    pyproject = root / "pyproject.toml"
    if not pyproject.exists():
        snippet = root / ".bughunt" / "configs" / "mutmut-snippet.toml"
        _write(snippet, _mutmut_block(source_paths, test_paths).lstrip())
        return ConfigArtifact("mutmut", str(snippet.relative_to(root)), "REVIEW", "no pyproject.toml; generated a mergeable config snippet")
    ...
    if begin in current_text and end in current_text:
        ...
        return ConfigArtifact(
            "mutmut", "pyproject.toml", "READY",
            f"managed config refreshed and verified; sources={required_sources!r}; tests={required_tests!r}; mutate_only_covered_lines=true",
        )
    ...
    if isinstance(existing, dict):
        ...
        configured_tests = _normalized_paths(existing.get("pytest_add_cli_args_test_selection"))
        gaps: list[str] = []
        if "paths_to_mutate" in existing:
            gaps.append("obsolete `paths_to_mutate` key; current mutmut uses `source_paths`")
        if "tests_dir" in existing:
            gaps.append("obsolete `tests_dir` key; current mutmut uses `pytest_add_cli_args_test_selection`")
        if existing.get("mutate_only_covered_lines") is False:
            gaps.append("`mutate_only_covered_lines=false` wastes mutation budget on code the suite never reaches; BugHunt reports uncovered code separately and recommends true")
        missing_sources = [item for item in required_sources if not _path_covered(configured_sources, item)]
        missing_tests = [item for item in required_tests if not _path_covered(configured_tests, item)]
        ...
        if gaps:
            return ConfigArtifact(
                "mutmut", "pyproject.toml", "REVIEW",
                "existing user-owned [tool.mutmut] preserved but coverage is incomplete: " + "; ".join(gaps),
            )
        return ConfigArtifact(
            "mutmut", "pyproject.toml", "EXISTING",
            f"existing user-owned config preserved and coverage-verified; sources={configured_sources!r}; tests={configured_tests!r}",
        )
    ...
    return ConfigArtifact("mutmut", "pyproject.toml", "READY", "added managed [tool.mutmut] source/test scoping; original backed up under .bughunt/backups")
...
def configure_custom_checks(root: Path, targets: Iterable[object]) -> ConfigArtifact:
    ...
    for target in targets:
        ...
        if not kind.startswith("custom-") or not runnable or not command or confidence != "high":
            continue
    ...
    lines = [begin, "# Regenerated by `bughunt configure --auto`; user-authored checks outside this block are preserved."]
    for check in checks:
        lines.extend([
            "",
            "[[custom.checks]]",
            ...
            "command = [" + ", ".join(json.dumps(x) for x in check["command"]) + "]",
    ...
    detail = f"{len(checks)} high-confidence repository-specific campaign(s) written to managed [[custom.checks]] entries"
    state = "READY" if checks else "READY"
...
def _manifest(artifacts: list[ConfigArtifact], root: Path) -> None:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps({"schema_version": 2, "artifacts": [asdict(a) for a in artifacts]}, indent=2) + "\n")
...
def _oxlint_config(react: bool) -> str:
    ...
    data = {
        ...
        "ignorePatterns": ["node_modules/**", "dist/**", "build/**", ".next/**", ".bughunt/**"],
    }
    return json.dumps(data, indent=2)
...
def _configure_technology_overlays(root: Path, config_dir: Path) -> list[ConfigArtifact]:
    ...
    if inv.has("sql"):
        ...
        put("SQLFluff", "sqlfluff.ini", _sqlfluff_config(dialect), f"correctness-only SQL rules; inferred dialect={dialect}")
    if inv.has("terraform"):
        put("TFLint", "tflint.hcl", _tflint_config(), "bundled Terraform language ruleset, recommended preset")
    if inv.has("docker"):
        put("Hadolint", "hadolint.yaml", _hadolint_config(), "Dockerfile correctness/best-practice checks; warning threshold")
    if inv.has("cpp"):
        put("clang-tidy", "clang-tidy.yaml", _clang_tidy_config(), "clang-analyzer + bugprone + concurrency checks; requires compilation database to execute")
    if inv.has("javascript-typescript"):
        put("Oxlint", "oxlintrc.json", _oxlint_config(inv.has("react")), "correctness+suspicious strict; pedantic/restriction/perf warnings; style disabled")
        put("ESLint", "eslint.config.mjs", _eslint_config(inv.has("typescript")), "correctness-first JS plus strict type-aware typescript-eslint when TypeScript is detected; project-owned ESLint configs remain supported")
    if inv.has("protobuf"):
        put("Buf", "buf.yaml", _buf_config(), "STANDARD lint + FILE breaking policy; used as an overlay when the repository has no stronger own policy")
    if inv.has("php"):
        put("PHPStan", "phpstan.neon", _phpstan_config(root), "maximum PHPStan rule level with first-party paths only")
    if inv.has("openapi"):
        out.append(ConfigArtifact("oasdiff", "history baseline", "READY" if inv.git_baseline else "REVIEW", f"validate current specs; breaking-change baseline={inv.git_baseline or 'unavailable'}"))
    if inv.has("protobuf"):
        out.append(ConfigArtifact("Buf compatibility", "history baseline", "READY" if inv.git_baseline else "REVIEW", f"breaking-change baseline={inv.git_baseline or 'unavailable'}"))
    return out
...
def _python_matrix_versions(root: Path) -> list[str]:
    ...
    if upper:
        ...
        versions = [v for v in versions if int(v.split(".")[1]) < minor or (inclusive and int(v.split(".")[1]) <= minor)]
...
def configure_all(
    ...
    *,
    ...
    project has none, after writing a backup. Existing user-owned mutmut configuration is preserved and audited; BugHunt-managed config is refreshed to coverage-guided mutation defaults.
    """
    ...
    def put(name: str, relative: str, content: str, detail: str) -> Path:
        path = config_dir / relative
        _write(path, content)
        artifacts.append(ConfigArtifact(name, str(path.relative_to(root)), "READY", detail))
        return path

    put("Ruff", "ruff.toml", _ruff_config(python_version, _existing(root, source_paths)), "ALL + preview rules; zero rule-family exclusions; tool/runtime paths excluded")
    put("basedpyright", "basedpyrightconfig.json", _basedpyright_config(config_dir, root, python_paths, python_version), "typeCheckingMode=all; Any/Unknown/unreachable/unsafe ignores are errors")
    put("mypy", "mypy.ini", _mypy_config(root, python_paths, python_version), "strict plus Any ban, unreachable analysis, and optional correctness error codes")
    put("ty", "ty.toml", _ty_config(), "all rules=error; strict equality/generic narrowing; generic type:ignore disabled")
    put("Pyrefly", "pyrefly.toml", _pyrefly_config(config_dir, root, python_paths, source_paths, python_version), "preset=all; no import-to-Any escape hatches; unannotated bodies checked; constants treated final")
    framework_inventory = discover_technologies(root, persist=True)
    pylint_plugins: list[str] = []
    if framework_inventory.has("django") and importlib.util.find_spec("pylint_django") is not None:
        pylint_plugins.append("pylint_django")
    if framework_inventory.has("odoo") and importlib.util.find_spec("pylint_odoo") is not None:
    ...
    put(
        ...
        + (f"; framework plugins: {', '.join(pylint_plugins)}" if pylint_plugins else ""),
    )
    put("Complexipy", "complexipy.toml", _complexipy_config(_existing(root, source_paths)), "cognitive complexity <=10; ignores disabled and ignored functions reported")
    put("Bandit", "bandit.yaml", _bandit_config(), "all installed security tests; first-party source roots only")
    ...
    if env_path is not None:
        artifacts.append(ConfigArtifact("environment contract", str(env_path.relative_to(root)), "READY", f"{len(env_uses)} statically discovered environment variable(s) documented with required/optional/default/type/unit/range/example evidence"))
    else:
        artifacts.append(ConfigArtifact("environment contract", ".env.example", "N/A", "no static environment-variable use detected; no .env.example generated"))

    sgconfig, rules = _astgrep_config()
    put("ast-grep", "sgconfig.yml", sgconfig, "generated structural rule pack + test directory")
    for filename, text in rules.items():
        put("ast-grep rule", f"ast-grep/rules/{filename}", text, "high-confidence structural bug rule")
    astgrep_tests = {
...
''',
    ...
    for filename, text in astgrep_tests.items():
        put("ast-grep rule test", f"ast-grep/tests/{filename}", text, "positive/negative regression fixtures for shipped structural rule")

    il = _import_linter_config(package_roots)
    if il:
        put("Import Linter", "importlinter.toml", il, f"root(s): {', '.join(package_roots)}; recursive acyclic-sibling invariant")
    else:
        artifacts.append(ConfigArtifact("Import Linter", "", "REVIEW", "could not safely infer an importable first-party package root"))
    ...
    if not pyre_path.exists():
        ...
        artifacts.append(ConfigArtifact("Pysa/Pyre", ".pyre_configuration", "READY", "base Pyre/Pysa source + model path generated"))
    else:
        artifacts.append(ConfigArtifact("Pysa/Pyre", ".pyre_configuration", "EXISTING", "existing project Pyre configuration preserved"))
    put("Pysa taint config", "pysa/taint.config", taint_text, "valid BugHunt source/sink namespace and taint rule")
    put("Pysa starter models", "pysa/bughunt.pysa", models_text, "valid model file; semantic app models intentionally require evidence")

    put("Hypothesis", "hypothesis_plugin.py", _hypothesis_plugin(), "2,000 examples, 100 state steps, shrinking + multiple bug reporting, no health-check suppression")
    put("coverage.py", "coverage.ini", coverage_config(_existing(root, source_paths)), "branch=true; missing lines/branches become first-class findings and feed risk/mutation prioritization")
    runtime_paths = write_runtime_plugins(root, _python_matrix_versions(root), _existing(root, test_paths))
    artifacts.append(ConfigArtifact("Blockbuster", str(runtime_paths[0].relative_to(root)), "READY", "autouse asyncio blocking-call detector for the runtime verification pass"))
    artifacts.append(ConfigArtifact("Python environment matrix", str(runtime_paths[1].relative_to(root)), "READY", "CPython 3.11-3.14 plus 3.14t when supported; deterministic Nox definition generated"))
    put("Schemathesis", "schemathesis.toml", _schemathesis_config(schemathesis_examples), "all generation modes, shrinking, x00/extra params, continue-on-failure, auto workers")
    ...
    artifacts.append(ConfigArtifact(
        "Semgrep", str(semgrep_rules.relative_to(root)), "READY",
    ...
    artifacts.append(ConfigArtifact("CodeQL", str(codeql_queries.relative_to(root)), "READY", "upstream security-and-quality suite plus local query directory"))

    artifacts.append(_ensure_mutmut_config(root, source_paths, test_paths))
    artifacts.extend(
        [
            ConfigArtifact("CrossHair", "bughunt.toml", "READY", "asserts + PEP316 + Deal + icontract; extended solver budgets in deep/all"),
            ConfigArtifact("Deal", "bughunt.toml", "READY", "Deal lint across first-party source roots"),
            ConfigArtifact("Vulture", "bughunt.toml", "READY", "confidence floor: PR 80, deep 40, all 0 for maximum dead-code recall"),
            ConfigArtifact("BugHunt policy pack", "built-in", "READY", "env contract/secrets, persistence-boundary coupling, export round-trip coverage, and implementation-coupled test heuristics"),
            ConfigArtifact("BugHunt complexity", "bughunt.toml", "READY", "cyclomatic + LOC + ABC + built asset budgets; complements Ruff/Pylint/Complexipy/Lizard/Radon"),
            ConfigArtifact("deptry", "bughunt.toml", "READY", "all dependency rules; first-party source roots with runtime/generated exclusions"),
            ConfigArtifact("Atheris", ".bughunt/generated/targets.json", "AUTO", "safe parser/decoder harness discovery follows config generation"),
        ]
    )
