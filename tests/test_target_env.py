...
def test_target_python_prefers_venv(tmp_path: Path):
...
def test_target_python_falls_back_without_venv(tmp_path: Path):
...
def test_target_executable_falls_back(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(
        "shutil.which",
...
def test_target_has_module_missing_interpreter(tmp_path: Path):
...
def test_exit_five_skips_with_skip_codes(tmp_path: Path):
...
def test_exit_five_errors_without_skip_codes(tmp_path: Path):
