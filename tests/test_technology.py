...
def test_detects_mixed_repository_capabilities(tmp_path: Path) -> None:
    ...
    (tmp_path / "migrations/001_create.sql").write_text("CREATE TABLE x (id SERIAL PRIMARY KEY);\n")
    ...
    for capability in (
        "github-actions", "shell", "dotenv", "openapi", "protobuf", "sql",
        "postgres-migrations", "docker", "terraform", "go", "rust", "cpp",
        "cpp-compile-db", "php", "javascript-typescript", "react",
    ):
...
def test_sql_dialect_inference_detects_postgres(tmp_path: Path) -> None:
    (tmp_path / "x.sql").write_text("CREATE TABLE x (id SERIAL PRIMARY KEY, body JSONB);\n")
...
def test_build_checks_marks_irrelevant_technology_na(tmp_path: Path) -> None:
    ...
    cfg = Config(tmp_path, {"project": {"source_paths": ["src"], "test_paths": [], "python_paths": ["src"]}, "profiles": {"pr": {"tools": ["compile"]}}, "timeouts": {"pr": 60}})
...
def test_parse_actionlint_json_lines() -> None:
    out = parse_actionlint('{"filepath":".github/workflows/ci.yml","line":7,"column":3,"kind":"expression","message":"unknown property"}\n', "", 1)
    assert out == [Finding(tool="actionlint", path=".github/workflows/ci.yml", line=7, column=3, code="expression", message="unknown property", severity="error")]


def test_parse_shellcheck_json1() -> None:
    data = {"comments": [{"file": "x.sh", "line": 3, "column": 2, "level": "warning", "code": 2086, "message": "Double quote"}]}
...
def test_parse_sqlfluff_json() -> None:
    data = [{"filepath": "q.sql", "violations": [{"code": "AM09", "description": "LIMIT without ORDER BY", "start_line_no": 4, "start_line_pos": 1}]}]
...
def test_parse_hadolint_and_tflint() -> None:
    h = parse_hadolint(json.dumps([{"file": "Dockerfile", "line": 2, "column": 1, "level": "warning", "code": "DL3003", "message": "Use WORKDIR"}]), "", 1)
    assert h[0].code == "DL3003"
    t = parse_tflint(json.dumps({"issues": [{"rule": {"name": "terraform_unused_declarations", "severity": "warning"}, "message": "unused", "range": {"filename": "main.tf", "start": {"line": 3, "column": 1}}}]}), "", 1)
...
def test_parse_clippy_compiler_message() -> None:
    payload = {
        ...
        "message": {
            ...
            "spans": [{"is_primary": True, "file_name": "src/lib.rs", "line_start": 9, "column_start": 5}],
        },
    }
...
def test_python_only_defenses_are_na_without_python(tmp_path: Path) -> None:
    ...
    raw["profiles"]["all"]["tools"] = ["compile", "mypy", "codeql", "pysa", "mutmut", "clippy"]
