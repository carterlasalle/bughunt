...
def test_coverage_parser_reports_line_and_branch_gaps(tmp_path: Path) -> None:
    ...
    report.write_text(json.dumps({
        "totals": {"percent_covered": 80, "covered_lines": 8, "missing_lines": 2, "num_branches": 4, "missing_branches": 1},
        "files": {"src/a.py": {"missing_lines": [7, 8], "missing_branches": [[6, 9]]}},
    }))
...
def test_seam_scanner_finds_schema_model_drift(tmp_path: Path) -> None:
    _python_repo(tmp_path)
    (tmp_path / "src/pkg/models.py").write_text("class User:\n    id: int\n    name: str\n")
    (tmp_path / "user.schema.json").write_text(json.dumps({
        "title": "User", "type": "object", "properties": {"id": {"type": "integer"}, "email": {"type": "string"}}
    }))
    findings = scan_seams(tmp_path, ["src"], ["tests"])
...
def test_local_schema_pairs_refuses_remote_and_resolves_local(tmp_path: Path) -> None:
    ...
    (tmp_path / "remote.json").write_text('{"$schema":"https://example.com/schema.json","x":1}')
    assert local_schema_pairs(tmp_path, ["local.json", "remote.json"]) == [("local.json", "schema.json")]


def test_correctness_floors_are_not_omitted_by_old_config(tmp_path: Path) -> None:
    cfg = Config(tmp_path, {"profiles": {"pr": {"tools": ["pytest"]}, "deep": {"tools": ["pytest"]}, "all": {"tools": ["pytest"]}}})
    assert {"coverage", "seam", "packaging", "runtime-types"} <= set(cfg.tools("pr"))
    assert {"pytest-random", "hypofuzz", "griffe", "type-disagreement"} <= set(cfg.tools("deep"))
    assert {"python-matrix", "timezone-matrix", "version-diff", "pynguin"} <= set(cfg.tools("all"))


def test_type_disagreement_is_first_class() -> None:
    results = [
        Result("mypy", "types", Status.FINDINGS, findings=[Finding("mypy", "bad", path="a.py", line=4)]),
...
def test_budgeted_search_timeout_is_not_infrastructure_error(tmp_path: Path) -> None:
    check = Check(
        "budget", "fuzz", [sys.executable, "-c", "import time; time.sleep(2)"],
        lambda _o, _e, _c: [], 1, tmp_path, timeout_is_success=True,
    )
...
def test_main_pytest_seed_is_recorded_not_hard_pinned_zero(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _python_repo(tmp_path)
    from bughunt import cli
    monkeypatch.setattr(cli, "executable", lambda *names: sys.executable if names and names[0] == "pytest" else None)
    monkeypatch.setattr(cli, "python_module_available", lambda name: name in {"hypothesis", "pytest_timeout", "pytest_randomly"})
    cfg = Config(tmp_path, {
        "project": {"source_paths": ["src"], "test_paths": ["tests"], "python_paths": ["src", "tests"]},
...
def test_correlations_and_risk_map_preserve_independent_evidence(tmp_path: Path) -> None:
    results = [
        Result("coverage", "coverage/branches", Status.FINDINGS, findings=[Finding("coverage", "branch", path="src/a.py", line=5, code="BHCOV002")]),
        Result("mypy", "types", Status.FINDINGS, findings=[Finding("mypy", "type", path="src/a.py", line=5, code="arg-type")]),
        Result("ruff", "lint", Status.FINDINGS, findings=[Finding("ruff", "bug", path="src/a.py", line=5, code="B012")]),
    ]
...
def test_version_differential_finds_observable_change(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    subprocess.run(["git", "config", "user.email", "bughunt@example.invalid"], cwd=tmp_path, check=True)
...
def test_logical_dedup_preserves_raw_evidence() -> None:
    results = [
        Result("mypy", "types", Status.FINDINGS, findings=[Finding("mypy", "bad type", path="src/a.py", line=9, code="arg-type")]),
        Result("basedpyright", "types", Status.FINDINGS, findings=[Finding("basedpyright", "bad type too", path="src/a.py", line=9, code="reportArgumentType")]),
    ]
...
def test_report_writes_checklist_and_deduplicated_queue(tmp_path: Path) -> None:
    cfg = Config(tmp_path, {"execution": {}})
    results = [
        Result("mypy", "types", Status.FINDINGS, findings=[Finding("mypy", "bad", path="src/a.py", line=3)]),
        Result("basedpyright", "types", Status.FINDINGS, findings=[Finding("basedpyright", "bad", path="src/a.py", line=3)]),
    ]
