...
def test_discovers_fastapi_and_parser(tmp_path: Path):
    ...
    targets = discover_all(tmp_path, ['src'], atheris_runs=123, schemathesis_examples=321)
    assert any(t.kind == 'schemathesis' and t.runnable and t.name == 'demo.api.app' for t in targets)
    fuzz = next(t for t in targets if t.kind == 'atheris' and t.name == 'demo.api.parse_packet')
    ...
    api = next(t for t in targets if t.kind == 'schemathesis' and t.name == 'demo.api.app')
...
def test_remote_openapi_is_not_auto_run(tmp_path: Path):
    ...
    schema.write_text('{"openapi":"3.1.0","info":{"title":"x","version":"1"},"servers":[{"url":"https://api.example.com"}],"paths":{}}')
...
def test_discovers_semantic_custom_campaigns_and_pysa_models(tmp_path: Path):
    ...
    targets = discover_all(tmp_path, ["src"], atheris_runs=100, schemathesis_examples=100)
    ...
    registry = __import__("json").loads((tmp_path / ".bughunt/generated/targets.json").read_text())
    models = registry["pysa_models"]
    assert any(model["kind"] == "source" and "get_input" in model["symbol"] for model in models)
    assert any(model["kind"] == "sink" and "run_command" in model["symbol"] for model in models)
...
def test_atheris_infers_expected_parser_rejections(tmp_path: Path):
    ...
    targets = discover_all(tmp_path, ["src"])
    target = next(t for t in targets if t.kind == "atheris" and t.name == "demo.parser.decode")
...
def test_export_import_names_generate_real_roundtrip_property_when_pure(tmp_path: Path) -> None:
    src = tmp_path / "src" / "demo"
    src.mkdir(parents=True)
    (src / "__init__.py").write_text("")
    ...
    targets = discover_all(tmp_path, ["src"])
    target = next(t for t in targets if t.kind == "custom-roundtrip" and "export_project" in t.name)
    assert target.runnable
